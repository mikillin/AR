...
def rotateSTL (filename = 'sun.stl', angle_x = 0, angle_y= 0, angle_z =0):
    # Load the STL file
    your_mesh = mesh.Mesh.from_file(filename)


    # Define rotation angles (in radians)
    angle_x = np.radians(angle_x)  # 45 degrees around the X-axis
    angle_y = np.radians(angle_y)  # 30 degrees around the Y-axis
    angle_z = np.radians(angle_z)  # 60 degrees around the Z-axis
    ...
    # Save the screenshot
    # image = plotter.show(screenshot="object.png" )
    # cv2.waitKey(0)
    # image = plotter.show()
